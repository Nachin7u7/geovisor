{include file="addfamily/form.css.tpl"}
<form method="POST"
      action="{$path_url}/{$subcontrol}_//saveaddfamily/"
...
{*            <input hidden type="text" id="phylum_id">*}
...
{*            </div>*}
    ...
    <div class="modal-footer p-2">
        {if $privFace.edit == 1}
            <button type="button" class="btn btn-success font-weight-bold" id="form_submit_{$subcontrol}_peque">
                <i class="la la-save"></i>{#glBtnSave#}</button>
        {/if}
        <button type="button" class="btn btn-light-success" id="form_close_{$subcontrol}_peque" data-dismiss="modal">
            <i class="la la-angle-double-left"></i>{#glBtnCloce#}</button>
    </div>
</form>

{include file="addfamily/form.js.tpl"}
